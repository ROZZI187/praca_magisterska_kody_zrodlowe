...
OUTPUT_DIR = Path(r"C:\Users\Mateusz-PC\Desktop\Praca magisterska\results_deep_lstm")
...
def audio_to_mfcc_sequence(y: np.ndarray) -> np.ndarray:
    ...
    # kształt dla LSTM: (time_steps, n_mfcc)
    mfcc = mfcc.T.astype(np.float32)

    # standaryzacja na poziomie próbki
    mfcc = (mfcc - np.mean(mfcc, axis=0, keepdims=True)) / (np.std(mfcc, axis=0, keepdims=True) + 1e-8)
...
# MODEL
def build_lstm_model(input_shape, n_classes: int) -> tf.keras.Model:
...
def plot_confusion_matrix(y_true, y_pred, class_names, split_name):
    ...
    plt.title(f"Macierz pomyłek - LSTM ({split_name})")
    plt.colorbar()
...
def plot_training_history(history):
    ...
    plt.xlabel("Epoka")
    plt.ylabel("Loss")
    plt.title("Przebieg funkcji straty - LSTM")
    plt.legend()
    plt.tight_layout()
    out1 = PLOTS_DIR / "lstm_loss_curve.png"
    ...
    plt.xlabel("Epoka")
    plt.ylabel("Accuracy")
    plt.title("Przebieg accuracy - LSTM")
    plt.legend()
    plt.tight_layout()
    out2 = PLOTS_DIR / "lstm_accuracy_curve.png"
...
def main():
    ...
    log(f"Kształt wejścia LSTM: {input_shape}")
    ...
    with open(REPORTS_DIR / "lstm_model_summary.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(summary_txt))

    callbacks = [
        tf.keras.callbacks.ModelCheckpoint(
            filepath=str(MODELS_DIR / "best_lstm.keras"),
    ...
    log(f"Trening LSTM zakończony. Czas treningu: {train_time_s:.2f} s")
    ...
    summary_path = REPORTS_DIR / "lstm_metrics_summary.csv"
    ...
    experiment_summary_path = REPORTS_DIR / "lstm_experiment_summary.csv"
